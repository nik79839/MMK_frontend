...
import { getCalculationResultInfoById, getCalculations, deleteCalculationById } from '../../../redux/main-reducer';
...
export const CalculationsUIDContainer: React.FC = () => { 
    ...
    const deleteCalculationById2 = (id: string) => {
        ...
        return <div className={s.full}>
                ...
                <TabPane tab={readyTabText} key="1">
                    <List loading = {spin} itemLayout="horizontal" dataSource={calculationReady} renderItem={(item) => (
                        <List.Item className={((item.id == calculationId) ? s.activeItemList : s.calculationsItems)} 
                            actions={[<DeleteOutlined style={{color: 'blue'}} onClick={() => deleteCalculationById2(item.id)}>Удалить</DeleteOutlined>]}>
                            <List.Item.Meta
            ...
            <TabPane tab={inProgressTabText} key="2">
                <List itemLayout="horizontal" dataSource={calculationProcess} renderItem={(item) => (
                        <List.Item className={s.calculationsItems} actions={[<CloseOutlined style={{color: 'red'}} onClick={() => deleteCalculationById2(item.id)}>Удалить</CloseOutlined >]}>
